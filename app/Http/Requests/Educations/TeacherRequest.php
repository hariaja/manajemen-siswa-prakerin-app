...
class TeacherRequest extends FormRequest
{
  ...
  public function rules(): array
  {
    return [
      ...
      'email' => [
        'required', 'email:dns',
        $this->method() == Constant::POST ? Rule::unique('users', 'email') : Rule::unique('users', 'email')->ignore($this->teacher->user_id)
      ],
      'phone' => [
        'required', 'numeric', 'min:12',
        $this->method() == Constant::POST ? Rule::unique('users', 'phone') : Rule::unique('users', 'phone')->ignore($this->teacher->user_id)
      ],
      'school_id' => [
