...
@section('content')
  <div class="block block-rounded">
    ...
    <div class="block-content">
      ...
      <form action="{{ route('schedules.update', $schedule->uuid) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="row justify-content-center">
          <div class="col-md-6">
            ...
            <div class="mb-4">
              ...
              <input type="text" name="title" id="title" value="{{ old('title', $schedule->title) }}" class="form-control @error('title') is-invalid @enderror" placeholder="{{ trans('Etc. Gelombang IV') }}">
              @error('title')
            ...
            <div class="mb-4">
              ...
              <input type="date" name="start" id="start" value="{{ old('start', date('Y-m-d', strtotime($schedule->start->toDateString()))) }}" class="form-control @error('start') is-invalid @enderror">
              @error('start')
                <div class="invalid-feedback">{{ $message }}</div>
              @enderror
            </div>
            ...
            <div class="mb-4">
              ...
              <input type="date" name="end" id="end" value="{{ old('end', date('Y-m-d', strtotime($schedule->end->toDateString()))) }}" class="form-control @error('end') is-invalid @enderror">
              @error('end')
                <div class="invalid-feedback">{{ $message }}</div>
              @enderror
            </div>
